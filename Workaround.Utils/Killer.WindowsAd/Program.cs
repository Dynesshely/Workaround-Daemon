...
public class Program : IIdentifier
{
    ...
    public static UtilAction GetOneKeyKiller(string name, string des, string key, string keyName)
    {
        return new UtilAction()
        {
            Name = name,
            Description = des,
            CanExecute = true,
            IsNeedToRestart = true,
            Execution = () => registryEditor.SetKey(key).DisableKey(keyName),
            Reopener = () => registryEditor.SetKey(key).EnableKey(keyName),
            EnabledChecker = () => registryEditor.SetKey(key).KeyToggled(keyName),
        };
    }

    public static UtilAction GetKeysKiller(string name, string des, string key, IEnumerable<string> keyNames)
    {
        return new UtilAction()
        {
            Name = name,
            ...
            CanExecute = true,
            IsNeedToRestart = true,
            Execution = () => registryEditor.SetKey(key).DisableKeys(keyNames),
